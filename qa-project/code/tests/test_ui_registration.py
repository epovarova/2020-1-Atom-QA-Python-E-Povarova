...
class TestRegistration(BaseCase):
    ...
    def test_positive_registration(self, correct_username):
        reg_page = self.main_page.go_to_registration()
        assert isinstance(
    ...
    def test_registration_without_sdet(self, correct_username):
        reg_page = self.main_page.go_to_registration()
        assert isinstance(
            reg_page.registration(correct_username, self.faker.email(), 1, 1, sdet=False),
            RegistrationPage)
        allure.attach(self.driver.get_screenshot_as_png(),
    ...
    def test_registration_without_name(self):
        ...
        reg_page = self.main_page.go_to_registration()
        assert isinstance(
            reg_page.registration("", self.faker.email(), 1, 1),
            RegistrationPage)
        allure.attach(self.driver.get_screenshot_as_png(),
    ...
    @pytest.mark.parametrize("email",
                             ['simple@example.com', 'very.common@example.com',
                              ...
                              'admin@mailserver1', 'example@s.example', '" "@example.org', '"john..doe"@example.org',
    ...
    def test_correct_email(self, email, mysql_orm_client, correct_username):
        ...
        try:
            assert isinstance(
                reg_page.registration(correct_username, email, 1, 1),
                WelcomePage)
    ...
    def test_registration_with_different_passwords(self, correct_username):
        reg_page = self.main_page.go_to_registration()
        reg_page.registration(correct_username, self.faker.email(), 1, 2).find(
            reg_page.locators.DIFFERENT_PASSWORDS)
        allure.attach(self.driver.get_screenshot_as_png(),
    ...
    def test_registration_with_few_errors(self):
        """SHOULD BE BEAUTIFUL ERROR MESSAGE"""
        reg_page = self.main_page.go_to_registration()
    ...
    def test_registration_without_password(self, correct_username):
        ...
        reg_page = self.main_page.go_to_registration()
        assert isinstance(
            reg_page.registration(correct_username, self.faker.email(), "", 1),
            RegistrationPage)
        allure.attach(self.driver.get_screenshot_as_png(),
    ...
    def test_registration_existing_user(self, config, add_user):
        ...
        reg_page.registration(name, email,
                              config['default_password'], config['default_password']).find(
            reg_page.locators.USER_ALREADY_EXIST)
        allure.attach(self.driver.get_screenshot_as_png(),
